...
// var chart_container=document.getElementsByClassName("chart-containe");
// const fluid = document.createElement('div');
// fluid.id=`fluid-mester-${4}`
// debugger

export const create_fluid=(fluidId,data)=>{
  var fluid_chart;
  const time_find_fluid= setInterval(() => {
    fluid_chart=document.getElementById(`fluid-meter-${fluidId}`)
    
    if (fluidId) {
      // debugger
      fluid_chart.innerHTML=null;
      var fm = new FluidMeter();
      fm.init({
        targetContainer: fluid_chart,
        fillPercentage: data,
        options: {
          fontSize: "30px",
          drawPercentageSign: true,
